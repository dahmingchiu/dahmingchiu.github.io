...
TLUserControls.prototype = {
	init: function() {
		...
		$(self.domOptions).click( function() {
			...
			self.jumpTo({ 
				pos: pos,
				callback: function() {
					self.blockClosed({ block: openBlock });
				}
			});			
			return false;
		});
	...
	doTranslation: function() {
		...
		if (trans && trans.userControls) {
			...
			$(menuItems[3]).css({ display: ((self.timeline.language == "english" || self.timeline.language == "english-common") ? "inline-block" : "none") });
	...
	showButton: function() {
		var self = this;
		$(self.domRoot).css({ display: "block" });
	},
	hideButton: function() {
		var self = this;
		$(self.domRoot).css({ display: "none" });
		self.close();
	},
	launch: function() {
		var self = this;
		...
		if ($.browser.msie) {
			$(self.domPanel).css({ display: "none" });
			setTimeout(function() {
				$(self.domPanel).css({ display: "block" });
			},500);
		}
		else {
			$(self.domPanel).css({ opacity: 0, display: "block" }).animate({ opacity: 1 },500, function() {});
		}
		$(self.domMenu).css({ display: "block" }).animate({ width: 325 }, 500, function() {
			self.launched = true;
			self.animating = false;
		});
	...
	close: function() {
		...
		if ($.browser.msie) {
			...
			$(self.domPanel).animate({ opacity: 0 },500, function() {
				$(this).css({ display: "none" });
			});
		}
		$(self.domMenu).animate({ width: 0 }, 500, function() { 
			$(this).css({ display: "none" }); 
			self.launched = false;
			self.animating = false;
			self.blockClosed({ block: self.selectedBlock });
		});
	},
	...
	initialiseBlockControllers: function() {
		...
		self.viewsByName["search"] = self.blockControllers[0] = new TLUCSearch({controller: self }).init();
		self.viewsByName["category"] = self.blockControllers[1] = new TLUCCategoryFilter({controller: self }).init();
		self.viewsByName["view"] = self.blockControllers[2] = new TLUCViewType({controller: self }).init();
		self.viewsByName["spacing"] = self.blockControllers[3] = new TLUCSpacing({controller: self }).init();
		self.viewsByName["zoom"] = self.blockControllers[4] = new TLUCZoom({controller: self }).init();
	},
	refreshTimelineView: function() {
		...
		self.mainController.updateViewsWithNewDateRangeAndZoom( { zoom: self.timeline.zoom } );
	...
	jumpTo: function(data) {
		...
		if (newPos > oldPos) {
			$(self.domBlocks[newPos]).css({display:"block"});
			$(self.domStage).animate({ marginTop: -self.carouselHeight}, duration, function() {
				$(self.domBlocks[oldPos]).css({ display: "none"});
			...
			$(self.domBlocks[newPos]).css({display:"block"});
			$(self.domStage).css({marginTop: -self.carouselHeight});
			$(self.domStage).animate({ marginTop: 0 }, duration, function() {
				$(self.domBlocks[oldPos]).css({ display: "none"});
				if (callback) {
					callback();	
				}
...
TLUCCategoryFilter.prototype = {
	...
	updateView: function() {
		...
		self.initialised = false;
		if (self.controller.isActiveBlock({ block: self })) {
			self.prepareForView();			
		}
	},
	initialise: function() {
		...
		$(self.domList).unbind("click").click( function(e) {
			...
			var domItem = AJKHelpers.getSelfOrFirstParantOfClass({ domEl: e.target, className: "tl-ccl-item"});
			if (domItem) {
				...
				if (iKey == "show-all") {
					...
					if (arriveCounter == 0) {
						// First category has been clicked, so we need to hide all other categories
	...
	generateCheckboxes: function() {
		...
		$.each(self.categories, function() {
			iHTML += '<li key="'+this.key+'" class="tl-ccl-item"><span style="background-color: #'+this.colour+'"></span><p>'+AJKHelpers.clipToMaxCharWords({ aString: this.title, maxChars: 16 })+'</p></li>';
		});
...
TLUCViewType.prototype = {
	...
	initialise: function() {
		...
		$(self.domItems).click( function() {
			self.updateViewTo({ view: $(this).attr("view") });
			return false;
		});
	...
	updateView: function() {
		var self = this;
		if (self.initialised) { 
			self.selectItem({item: self.timeline.viewTypeValue});
		}
	},
	prepareForView: function() {
		var self = this;
	...
	updateViewTo: function(data) {
		...
		// Hide the slider pointers for hidden categories
		if (self.controller.blockControllers[1].activeCounter > 0) {
			$.each(self.mainController.markers, function() {
				if (!this.category || self.timeline.categoriesByKey[self.mainController.categoriesKeyPrefix+this.category.id].hide) {
					$(this.domSliderPoint).css({visibility:"hidden"});
				}
			});
		}
	},
...
TLUCZoom.prototype = {
	...
	initialise: function() {
		...
		$(self.zoomSelect).unbind("change").change( function() {
			self.updateZoomTo({ zoom: $(this).val() });
	...
	zoom: function(data) {
		...
		if (self.mainController.selectedView.availableScales[cPos]) {
			self.updateZoomTo({ zoom: self.mainController.selectedView.availableScales[cPos].name });
			self.zoomSelectReplacer.selectItemFromVal({ val: self.timeline.zoom });
		}
	...
	updateZoomTo: function(data) {
		var self = this;
	 	self.mainController.updateViewsWithNewDateRangeAndZoom({ zoom: data.zoom });
	},
	prepareForView: function() {
		var self = this;
		if (!self.initialised) {
...
TLUCSpacing.prototype = {
	...
	initialise: function() {
		...
		$(self.domSelect).change( function() {
			self.updateSpacing({ spacing: $(this).val() });
		});
...
TLUCSearch.prototype = {
	...
	updateSearchFilterText: function() {
		...
		if (self.filterText) {
			...
			$(self.domEveryoneMessage).css({ display: "none" });
			...
			$(self.domFilterMessage).css({ display: "inline" });
		}
		else {
			$(self.domFilterMessage).css({ display: "none" });
			$(self.domEveryoneMessage).css({ display: "inline" });
		}
	},
	filterStories: function() {
		...
		if (firstStory) {
			self.mainController.focusMarker({ marker: firstStory });
		}
		self.updateSearchFilterText();
	},
	showAllStories: function() {
		var self = this;	
		$.each(self.mainController.markers, function() { this.searchShow() });
 		self.updateSearchFilterText();
	},
	viewHasEnded: function() {
		var self = this;
...
var TLSingleTimelineLicense = true;
...
return"http://flickr.com";},convertNumToXDecimalPlaces:function(data){var num=data.num;var cnvInt=data.x*10;return parseInt(num*cnvInt)/cnvInt;},stringRepeat:function(data){var aString=data.aString;var multiplier=data.multiplier;return new Array(multiplier+1).join(aString);},convertHexColourToRGB:function(data){var hexColour=data.hexColour;var cutHex=function(h){return(h.charAt(0)=="#")?h.substring(1,7):h}
var hexToR=function(h){return parseInt((cutHex(h)).substring(0,2),16)}
var hexToG=function(h){return parseInt((cutHex(h)).substring(2,4),16)}
var hexToB=function(h){return parseInt((cutHex(h)).substring(4,6),16)}
return{r:hexToR(hexColour),g:hexToG(hexColour),b:hexToB(hexColour)}},generateAudioEmbedHTML:function(data){var self=this;var audioSrc=data.src;var type=data.type;var audioId=data.audioId;var colour=data.colour;if(type=="soundcloud"){var iHTML='<object height="100%" width="100%" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000">';iHTML+='<param name="movie" value="http://player.soundcloud.com/player.swf?url='+audioSrc+'&auto_play=true&show_artwork=true&theme_color=000000&color='+colour+'"></param>';iHTML+='<param name="allowscriptaccess" value="always"></param>';iHTML+='<embed allowscriptaccess="always" height="100%" src="http://player.soundcloud.com/player.swf?url='+audioSrc+'&auto_play=true&show_artwork=true&theme_color=000000&color='+colour+'" type="application/x-shockwave-flash" width="100%"></embed>';iHTML+='</object>';return iHTML;}
...
$(domImage).css({width:adjustedAgainDimensions.width+"px",height:adjustedAgainDimensions.height+"px",left:adjustedAgainDimensions.xOffset+"px",top:adjustedAgainDimensions.yOffset+"px"});if(returnScaledImageSize){return adjustedDimensions;}},customCreateClickableLinks:function(data){var aString=data.aString;var extra=(data.extra)?data.extra:"";var myRegex=/\[(.*)\]\((.*)\)/gim
...
AJKImageLoader.prototype={init:function(){var self=this;var anImage=new Image();anImage.src=self.imageUrl;if(anImage.complete){if(self.loadCallback){self.loadCallback({image:anImage});}}
...
AJKImage.prototype={init:function(){var self=this;$(this.domHolder).css({width:this.displaySize.width,height:this.displaySize.height});$(self.domHolder).append(self.domImage);new AJKImageLoader({imageUrl:$(self.domImage).attr("src"),loadCallback:function(){setTimeout(function(){self.size={width:$(self.domImage).width(),height:$(self.domImage).height()}
...
else{$(window).bind("blue",function(){self.informObserversOfBlur();});}},endEvent:function(){var self=this;if(!self.postMessageAvailable){$(self.domBody).unbind("mouseleave");}
else{$(window).unbind("blur");}},informObserversOfBlur:function(){var self=this;$.each(self.observers,function(){if(this.windowDidBlur){this.windowDidBlur();}});},registerAsObserver:function(dataObj){var self=this;var observer=dataObj.observer;self.observers=AJKHelpers.removeItemFromArray({anArray:self.observers,item:observer})
...
AJKDraggable.cancelClick=false;AJKDraggable.prototype={init:function(){var self=this;$(self.domDragEl).mousedown(function(e){self.initiateDrag({event:e});});return self;},kill:function(){var self=this;$(self.domDragEl).unbind();theAJKMouseMoveEvent.removeAsObserver({observer:self});theAJKWindowBlurEvent.removeAsObserver({observer:self});},initiateDrag:function(dataObj){var self=this;var event=dataObj.event;self.currentlyDragging=true;self.domDraggable=(self.clone)?$(this.domDragEl).clone().get()[0]:this.domDragEl;if(self.cloneClass){$(self.domDraggable).addClass(self.cloneClass);}
...
self.savedCoords=newMouseCoords;var timeoutTime=($.browser.msie)?10:0;setTimeout(function(){$(self.domDraggable).css({top:newPos.y,left:newPos.x});if(self.mouseMoveFunc){self.mouseMoveFunc({dragElPos:newPos});}
if(self.type&&theAJKMouseMoveEvent.observersOfType[self.type]){$.each(theAJKMouseMoveEvent.observersOfType[self.type],function(){if(this.dragDidMove){this.dragDidMove({coords:newMouseCoords,data:self.data});}});}},timeoutTime);}}
...
var AJKSelectReplacer=function(data){var self=this;self.domRoot="";self.domSelect=data.domSelect;self.createItemFunc=data.createItemFunc;self.itemSelectedClass=data.itemSelectedClass;self.changeFunc=data.changeFunc;self.items=new Array();self.keyPrefix="ajk-sr-"
...
marker.setSliderPointPosition({position:{left:leftOffset,top:topOffset}});marker.setDomSliderPoint({domEl:domMarker});marker.setDomTooltipDomContainer({domEl:self.domRoot});return domMarker;},getLeftOffsetPercentForDate:function(data){var self=this;var date=data.date;var startTimeNum=self.startDate.getTime();var diff=self.endDate.getTime()-startTimeNum;var leftOffset=(date.getTime()-startTimeNum)/diff*100;return leftOffset;},displayCanvasScale:function(){var self=this;var generateYearScales=function(data){var yearsPerSection=data.yearsPerSection;self.sliderScaleSectionWidth=self.sliderScaleWidth/(theTLSettings.timeInfo.years)*yearsPerSection;var yearRounding=(self.timeline.markerSpacing=="equal")?1:yearsPerSection;var tempDate=AJKHelpers.getEmptyDate();tempDate.setTime(theTLSettings.timeInfo.start.getTime());tempDate=AJKHelpers.getFirstDayOfYearDateForDate({date:tempDate});var tmpFullYear=tempDate.getFullYear();if(tmpFullYear>=0){tempDate.setFullYear(tmpFullYear-(tmpFullYear%yearsPerSection));}
else{tempDate.setFullYear(-(Math.abs(tmpFullYear)+yearsPerSection-(Math.abs(tmpFullYear)%yearsPerSection)));}
self.sliderScaleFirstSectionPosition=self.convertDateToFraction({date:tempDate})*self.sliderScaleWidth;self.sliderScaleOffsetAdjust=0;self.sliderScaleSectionPts=(yearsPerSection<5)?24:20;self.scaleDateFormaterMain=function(data){var aDate=data.date;var fullYear=aDate.getFullYear();if(aDate.getMonth()==11){aDate.setMonth(0);aDate.setFullYear(++fullYear);}
...
var generateMonthScales=function(data){var monthsPerSection=data.monthsPerSection;self.sliderScaleSectionWidth=self.sliderScaleWidth/(theTLSettings.timeInfo.months)*monthsPerSection;var tempDate=AJKHelpers.getEmptyDate();tempDate.setTime(theTLSettings.timeInfo.start.getTime());tempDate=AJKHelpers.getFirstDayOfMonthDateForDate({date:tempDate});tempDate.setMonth(0);self.sliderScaleFirstSectionPosition=self.convertDateToFraction({date:tempDate})*self.sliderScaleWidth;self.sliderScaleOffsetAdjust=(theTLSettings.timeInfo.years<1)?10:8;self.sliderScaleSectionPts=(theTLSettings.timeInfo.years>0.8)?16:30;self.scaleDateFormaterMain=function(data){var aDate=data.date;var thisDay=aDate.getDate();if(thisDay=="31"){aDate.setTime(aDate.getTime()+AJKHelpers.dateOneDayInMS);}
...
for(var offset=self.sliderScaleFirstSectionPosition;offset<(self.sliderScaleWidth+50);offset+=self.sliderScaleSectionWidth/self.sliderScaleSectionPts){if(counter%self.sliderScaleSectionPts==0){self.scaleCanvas.fillRect(offset,canvasHeight-14,bigLineWidth,13);self.scaleCanvas.fillRect(offset,0,bigLineWidth,5);if(!self.scaleTimesTextHasBeenDisplayed){var aDate=AJKHelpers.getEmptyDate();if(self.timeline.markerSpacing=="equal"&&self.items.length>0){var datePos=(offset+self.sliderScaleOffsetAdjust)/self.sliderScaleWidth;var aDate=self.getEquallySpacedDateForOffset({offset:datePos});}
...
var domTimer=$('<h5 style="color: #'+self.timeline.sliderTextColour+'">'+self.scaleDateFormaterMain({date:aDate})+"</h5>").get()[0];$(domTimer).css({left:offset/self.sliderScaleWidth*100+"%"});$(domDiv).append(domTimer);}}
...
TLSliderDragger.prototype={init:function(){var self=this;self.updateDraggerSize();var draggerTop=parseInt($(self.domEl).css("top"));self.draggable=new AJKDraggable({domDragEl:self.domEl,limitFunc:function(){var limit={minX:0,maxX:self.getSliderWidth()-self.width,minY:draggerTop,maxY:draggerTop}
return limit;},mouseMoveFunc:function(data){self.draggedToLeftPos(data);},dragDidStartFunc:function(){self.beingDragged=true;theTLSettings.lastSelectedStory="";},dragDidEndFunc:function(){self.beingDragged=false;self.dragEndedCallback();}}).init();return self;},updateColour:function(data){var self=this;var colour="#"+data.colour;$(self.domInner).css({borderColor:colour});$(self.domInner).find(".tlsd-corner").css({backgroundColor:colour});},kill:function(){var self=this;self.draggable.kill();},draggedToLeftPos:function(data){var self=this;var dragElPos=data.dragElPos;self.dragCallback({xPos:dragElPos.x});},updateDraggerSize:function(){var self=this;self.width=parseInt(theTLSettings.windowSize.width/self.getStageWidth()*self.getSliderScaleWidth());self.width=parseInt(self.width/2)*2+self.padding*2;self.width=(self.width>self.minWidth)?self.width:self.minWidth;self.width=(self.width>self.getSliderScaleWidth())?self.getSliderScaleWidth():self.width;$(self.domEl).css({width:self.width});$(self.domInner).css({width:self.width-24});},setLeftPosition:function(data){var self=this;var pos=data.pos;var animate=data.animate;var animSpeed=(animate)?theTLSettings.animateTime:0;$(self.domEl).stop().animate({left:pos},animSpeed);}}
...
TLMainController.prototype={init:function(){var self=this;AJKHelpers.isScriptable=self.timeline.scriptable;self.sortCategories();self.initBrowserStyles();self.initFontStyles();self.initLightboxStyles();if(self.timeline.language!="english"){self.initLanguage();}
...
$(self.domStageHolder).css({backgroundColor:"#"+self.timeline.backgroundColour});theAJKWindowResizeEvent.registerAsObserver({observer:self});self.initialiseCategories();if(typeof TLFeedController!="undefined"){self.feedController=new TLFeedController({controller:self,feeds:self.timeline.feeds,feedsLoadedCallback:function(){if(self.timeline.feeds.length>0){self.refreshTimelineAfterFeedChange();self.setupInitialDate({ignoreHash:!self.isInitialHashDate});}
...
setTimeout(function(){self.initCopyable();},200);return self;},initLazyLoading:function(){var self=this;(function(){var thisFunc=arguments.callee;var counter=0;var minPos=-parseInt(self.selectedView.stageLeftOffset)-500;var maxPos=minPos+theTLSettings.windowSize.width+1000;$.each(self.markers,function(){var thisMarker=this;if(!thisMarker.lazyImageLoaded&&thisMarker.displayImage){counter++;if(thisMarker.leftOffset>minPos&&thisMarker.leftOffset<maxPos){thisMarker.lazyImageLoaded=true;new AJKImageLoader({imageUrl:thisMarker.getDisplayImage().src,loadCallback:function(){thisMarker.updateDisplayImage();thisMarker.updateMainImage();}}).init();}}});if(counter||(!self.initialFeedsLoaded&&self.timeline.feeds.length>0)){setTimeout(function(){thisFunc()},250);}})();},initTimelineControls:function(){var self=this;self.userControlsController=new TLUserControls().init();},getBGImageCredit:function(){var self=this;var ret=self.timeline.backgroundImageCredit;if(self.timelineBackgroundImage&&self.timelineBackgroundImage.indexOf("flickr.com")!=-1){ret=(ret)?ret+" - ":"Background Photo: ";ret+='<a target="_blank" href="'+AJKHelpers.getFKRPhotoPageFromImageSrc({src:self.timelineBackgroundImage})+'">View on Flickr</a>';}
return ret;},initDurationTextColourStyle:function(){var self=this;if(self.domDurStyles){$(self.domDurStyles).remove();}
...
ins+='</style>';self.domFontStyles=$(ins).get()[0];$("head").append(self.domFontStyles);}},initLightboxStyles:function(){var self=this;if(self.timeline.lightboxStyle==1){$(self.domContentHolder).addClass("tl-lightbox-2");}},initBrowserStyles:function(){var self=this;var browserClass="browser-type-";browserClass+=($.browser.mozilla)?"firefox":"";browserClass+=($.browser.opera)?"opera":"";browserClass+=($.browser.webkit)?"webkit":"";browserClass+=($.browser.isIE9)?"ie9":"";$(self.domMainHolder).addClass(browserClass);},updateShowAuthor:function(){var self=this;if(self.timeline.showGroupAuthorNames==1){$(self.domMainHolder).removeClass("tl-ch-author-hide");}
else{$(self.domMainHolder).addClass("tl-ch-author-hide");}},customiseHeader:function(){var self=this;$(self.domTimelineHeader).css({backgroundColor:"#"+self.timeline.headerBackgroundColour});$(self.domTimelineTitle).css({color:"#"+self.timeline.headerTextColour});$(self.domTimelineHeader).find(".main-menu a").css({color:"#"+self.timeline.headerTextColour});},initLanguage:function(){var self=this;var trans=TLTranslation[self.timeline.language];if(trans){if(!trans.userControls){trans.userControls=TLTranslation["english"].userControls;}
...
self.timeline.zoom=self.timeline.savedZoom;self.updateViewsWithNewDateRangeAndZoom({zoom:self.timeline.zoom});self.flushSize();if(self.adminController&&!self.user.secretUser){self.adminController.storyListController.resetWithNewListItems({listItems:self.timeline.markers});self.adminController.refreshTimelineZoomSelector()}},keyEventTookPlace:function(data){var self=this;var key=data.key;var mode=data.mode;if(key==39&&mode=="keyup"){self.animateToStory({direction:"next"});}
else if(key==37&&mode=="keyup"){self.animateToStory({direction:"previous"});}},animateToStory:function(data){var self=this;var direction=data.direction;if(self.contentPanelController.panelVisible){if(direction=="next"&&self.contentPanelController.nextStory&&self.contentPanelController.selectedStory.extraInfoLoaded){if(!self.contentPanelController.lastStoryReached){self.focusMarker({marker:self.contentPanelController.nextStory});self.contentPanelController.displayForStory({story:self.contentPanelController.nextStory});}}
...
else{if(self.timeline.markerSpacing=="equal"){var nearestMarkerIndex=self.selectedView.getClosestMarkerIndexToDate({date:theTLSettings.currentDate});if(nearestMarkerIndex<0){var nextStory=self.markers[0];var prevStory=false;}
...
else{var counter=0;var storyFound=false;while(self.markers[counter]&&!storyFound){if(self.markers[counter].startDate.getTime()==theTLSettings.currentDate.getTime()){var nextStory=self.markers[counter+1];var prevStory=self.markers[counter-1];storyFound=true;}
...
else if(direction=="previous"&&prevStory){self.focusMarker({marker:prevStory});self.lastKeyboardSelectedStory=prevStory;}},setupInitialDate:function(data){var self=this;var initialStartTime="";var initialStartMarker="";var numMarkers=self.markers.length;var ignoreHash=(data&&data.ignoreHash);if(self.timeline.initialFocus=="today"){initialStartTime=new Date();}
...
theTLSettings.timeInfo.markerDisplayDateFormat=markerDateFormat;},flushSize:function(){var self=this;self.windowDidResize({newSize:{width:theTLSettings.windowSize.width,height:theTLSettings.windowSize.height}});},windowDidResize:function(dataObj){var self=this;var newSize=dataObj.newSize;self.resizeContent({newSize:newSize});},resizeContent:function(data){var self=this;var newSize={width:data.newSize.width,height:data.newSize.height}
...
$.each(inactiveMarkers,function(){this.makeInvisible();});$.each(self.activeMarkers,function(){this.makeVisible();});},addNewCategory:function(data){var self=this;var category=data.category;self.timeline.categories.push(category);self.initialiseCategory({category:category});self.sortCategories();},sortMarkersList:function(){var self=this;self.markers.sort(function(a,b){return(a.startDate.getTime()>b.startDate.getTime())?1:-1;});var counter=0;$.each(self.markers,function(){this.orderIndex=counter++;});if(self.timeline.viewType=="category-band"){self.sortMarkersByCategoryList();}},sortMarkersByCategoryList:function(){var self=this;self.markersByCategory=[];$.each(self.timeline.categories,function(){if(!self.markersByCategory[this.key]){self.markersByCategory[this.key]={numItems:0,markers:[]};}});$.each(self.markers,function(){var thisMarker=this;if(!self.markersByCategory[thisMarker.category.key]){self.markersByCategory[thisMarker.category.key]={numItems:0,markers:[]};}
self.markersByCategory[thisMarker.category.key].markers.push(thisMarker);thisMarker.categoryIndex=self.markersByCategory[thisMarker.category.key].numItems;thisMarker.categoryIndex=self.markersByCategory[thisMarker.category.key].numItems++;});},initCopyable:function(){var self=this;self.domCopyButton=$("#tl-copy-timeline-button").get()[0];self.domCopyText=$(self.domCopyButton).find("span").get();if(self.timeline.embed=="true"){self.loadUserCookie();}
self.updateCopyButton();$(self.domCopyButton).click(function(){var cookieString=self.user.id+self.cookieSeparator;cookieString+=self.user.username+self.cookieSeparator;cookieString+=self.user.verifyCode+self.cookieSeparator;cookieString+=self.timeline.id
...
else{lastFoundItem=self.items[self.items.length-1];var viewStageWidth=self.width;if(lastFoundItem.startDate.getTime()!=self.endDate.getTime()){var extra=(viewStageWidth-lastFoundItem.leftOffset)*(aDate.getTime()-lastFoundItem.startDate.getTime())/(self.endDate.getTime()-lastFoundItem.startDate.getTime());}
else{extra=0;}
offset=lastFoundItem.leftOffset+extra;}
return offset;},createCategoryBands:function(){var self=this;var spaceRemaining=100;var ourCategories=[];var totalSize=0;$.each(self.timeline.categories,function(){if(!this.hide&&!(this.autoGenerated&&theTLMainController.markersByCategory[this.key]&&theTLMainController.markersByCategory[this.key].markers.length==0)){ourCategories.push(this);totalSize+=parseInt(this.size,10);}});var itemsRemaining=ourCategories.length;var remainingSize=totalSize;if(self.timeline.categories.length==0){ourCategories=[theTLMainController.defaultCategory];itemsRemaining=1;remainingSize=theTLMainController.defaultCategory.size;}
...
$(self.domEl).css({width:self.width}).append(aDiv);},currentDateWasUpdatedTo:function(data){var self=this;var currentDate=data.date;var animate=(data.extraInfo)?data.extraInfo.animate:false;if(animate){self.animateViewToDate({date:currentDate,speed:theTLSettings.animateTime});}
...
else{self.slideViewToDate({date:currentDate,instantly:!animate});}
...
self.realStageLeftOffset+=(self.stageLeftOffset-self.realStageLeftOffset)/self.easingFraction;$(self.domEl).css({left:self.realStageLeftOffset});if(parseInt(self.stageLeftOffset-self.realStageLeftOffset)!=0){var thisFunc=arguments.callee;self.animationTimeout=setTimeout(function(){thisFunc();},self.easeTimeoutTime);}
else{self.animationTimeout="";}})();}},windowDidResize:function(data){var self=this;setTimeout(function(){self.slideViewToDate({date:theTLSettings.currentDate,instantly:true});self.generateMinMaxStagePositionsFromVisibleDates();},1);},updateScaleColour:function(data){var self=this;var colour=data.colour;$.each(self.viewScaleBlocks,function(){this.updateScaleColour(data);});}}
...
var insertHTML='<div class="tl-view-category-band-label" style="top: '+(self.height+self.top)+'%;"><h4 style="background-color: #'+highlightColor+';">'+categoryTitle+'</h4></div>';self.domLabel=$(insertHTML).get()[0];},refreshFromCategory:function(){var self=this;$(self.domLabel).find("h4").css({backgroundColor:"#"+self.category.colour}).text(self.category.title);$(self.domRoot).css({borderColor:"#"+self.category.colour});$(self.domRoot).find(".tl-vcb-inner, tl-vcb-scale").css({backgroundColor:"#"+self.category.colour});}}
...
return self;},setLeftPosition:function(data){var self=this;var position=data.position;$(self.domEl).css({left:position});},updateScaleColour:function(data){var self=this;self.colour=data.colour;$(self.domColourBlock).css({background:"#"+self.colour});}}
...
else{return AJKHelpers.formatFullYearForDate({date:aDate});}},displayDateFormat:"MMMM YYYY",numItems:12,segmentWidth:64}
...
retDate.setFullYear(roundedYear);retDate.setMonth(1);retDate.setDate(1);retDate.setHours(0);retDate.setMinutes(0);retDate.setSeconds(0);return retDate;},getDateRangeInMS:function(data){var blockStartDate=data.blockStartDate;var retDate=AJKHelpers.createDateWithTime({time:blockStartDate.getTime()});retDate.setFullYear(blockStartDate.getFullYear()+10);return retDate.getTime()-blockStartDate.getTime();},getTextForDate:function(data){var aDate=data.date;return AJKHelpers.formatFullYearForDate({date:aDate});},displayDateFormat:"YYYY",numItems:10,segmentWidth:64}
...
return self;},initialiseMedia:function(){var self=this;self.media=self.media.sort(function(a,b){return(a.orderIndex<b.orderIndex)?-1:(a.orderIndex==b.orderIndex)?0:1;});self.images=new Array();self.videos=new Array();self.audio=new Array();self.mediaByKey=new Array();var listPos=0;$.each(self.media,function(){if(this.src&&this.src.charAt(0)!="h"&&this.src.charAt(0)!="/"){this.src="http://"+this.src;}
...
self.isFlickrImage=(self.displayImage.src&&self.displayImage.src.indexOf("flickr.com")!=-1);},updateDisplayImage:function(){var self=this;if(self.isFlickrImage){$(self.domMainImageHolder).addClass("ig-inner-flickr");}
else{$(self.domMainImageHolder).removeClass("ig-inner-flickr");}
$(self.domMainImageHolder).empty().append('<img src="'+self.getDisplayImage().src+'" alt="" />');$(self.domMainImageHolder).append('<div class="ig-inner-image-mask"></div>');self.domMainImage=$(self.domMainImageHolder).find("img").get()[0];},loadExtraInfo:function(data){var self=this;var callback=(data)?data.callback:"";if(!self.extraInfoLoaded&&!self.extraInfoIsBeingLoaded&&self.id){vars={}
...
insertHTML+='<h4>'+self.headline+'</h4>';insertHTML+='</div>';insertHTML+='<div class="tl-s-t-bottom"></div>';insertHTML+='</div>';self.domSliderToolTip=$(insertHTML).get()[0];self.domSliderToolTipHeadline=$(self.domSliderToolTip).find("h4").get()[0];self.domSliderToolTipDate=$(self.domSliderToolTip).find("h5").get()[0];$(self.domSliderToolTipDate).css({color:"#"+self.category.colour});},showToolTip:function(data){var self=this;var sliderScaleWidth=data.sliderScaleWidth;var scaleLeftOffset=data.scaleLeftOffset;var sliderWidth=data.sliderWidth;if(!self.toolTipVisible){self.toolTipVisible=true;if(!self.domSliderToolTip){self.createToolTip();}
...
return self.domRoot;},getDisplayImage:function(){var self=this;return{src:AJKHelpers.getTimelineImageUrl({src:self.displayImage.src})};},refreshPositionAndSize:function(){var self=this;self.refreshSizeClass();$(self.domStandardPin).css({left:(self.standardPinPosition[this.vSize]-self.horizAdjustment)+"px"});self.setMarkerHorizontalAndVerticalPosition();},updateMainImage:function(){var self=this;if($.browser.opera||$.browser.msie){setTimeout(function(){self.updateMainImageFunc()},10);}
else{self.updateMainImageFunc()}},updateMainImageFunc:function(){var self=this;var newImage=$(self.domMainImage).clone().get()[0];$(newImage).css({height:"auto",width:"auto"});$(self.mainController.domImagePreloader).append(newImage);var imgLoadedFunc=function(){var thisHeight=self.mainImageHeight=newImage.height;var thisWidth=self.mainImageWidth=newImage.width;$(newImage).remove();self.mainImageSize={width:thisWidth,height:thisHeight}
...
insertHTML+='<div class="ig-inner-image-mask"></div>';insertHTML+='</div>';insertHTML+='</div>';return insertHTML;},setMarkerHorizontalAndVerticalPosition:function(){var self=this;var extraBottom=(self.timeline.viewType=="category-band")?5:0;$(self.domRoot).css({left:self.leftOffset+self.horizAdjustment,bottom:(extraBottom+self.verticalPos*8.75)+"%"});if(self.domDurationBar&&self.vSize=="duration-normal"&&self.startDate.getTime()!=self.endDate.getTime()){$(self.domDurationBar).css({width:self.durationBarWidth});}},showExtraInfo:function(){var self=this;self.mainController.contentPanelController.displayForStory({story:self});},searchShow:function(){var self=this;$(self.domRoot).removeClass("tl-sb-search-hide");$(self.domSliderPoint).removeClass("tl-sp-search-hide");},searchHide:function(){var self=this;$(self.domRoot).addClass("tl-sb-search-hide");$(self.domSliderPoint).addClass("tl-sp-search-hide");},focus:function(){var self=this;self.loadExtraInfo();if(self.timeline.viewType=="category-band"){$(self.domRoot).addClass("tl-story-block-category-hover");}
$(self.domRoot).css({zIndex:2});if(self.vSize=="miniature"){$(self.domRoot).removeClass("tl-sb-"+self.vSize+"-height");$(self.domRoot).addClass("tl-sb-very-low-height").addClass("tl-sb-very-low-height-no-image");}},unfocus:function(){var self=this;if(self.timeline.viewType=="category-band"){$(self.domRoot).removeClass("tl-story-block-category-hover");}
$(self.domRoot).css({zIndex:1});if(self.vSize=="miniature"){$(self.domRoot).removeClass("tl-sb-very-low-height").removeClass("tl-sb-very-low-height-no-image");;$(self.domRoot).addClass("tl-sb-"+self.vSize+"-height");}},makeInvisible:function(){var self=this;$(self.domRoot).css({visibility:"hidden"});$(self.domSliderPoint).css({visibility:"hidden"});},makeVisible:function(){var self=this;$(self.domRoot).css({visibility:"visible"});$(self.domSliderPoint).css({visibility:"visible"});},deleteSelf:function(){var self=this;$(self.domSliderToolTip).remove();$(self.domRoot).remove();$(self.domSliderPoint).remove();}}
...
TLContentPanelController.prototype={init:function(){var self=this;if(self.timeline.openReadMoreLinks){$(self.domReadMoreButton).attr("target","_blank");}
...
else{self.hide();}});$(self.domStartTimelineButton).click(function(){self.hide();return false;});self.contentScroller=new AJKContentScrollerController({domRootEl:self.domTextContentCarousel}).init();self.selectedGallerySelector=self.domImageGallerySelector;$(self.domMediaSelectors).each(function(){var thisSelector=this;var thisSelectorType=$(this).attr("rel");$(this).find("a").click(function(){var numItems=parseInt($(this).find("span").text());if(numItems==0){return false;}
...
return false;});});$(self.domCloseButton).click(function(){self.hide();return false;});$(self.domCloseVideoButton).click(function(){self.removeVideo();return false;});$(self.domNextStory).click(function(){if(self.nextStory&&!self.contentCarouselAnimating){self.mainController.focusMarker({marker:self.nextStory,cancelUpdateHash:true});self.animateContentToStory({story:self.nextStory,direction:"next"});}
return false;});$(self.domPrevStory).click(function(){if(self.prevStory&&!self.contentCarouselAnimating){self.mainController.focusMarker({marker:self.prevStory,cancelUpdateHash:true});self.animateContentToStory({story:self.prevStory,direction:"previous"});}
return false;});return self;},animateContentToStory:function(data){var self=this;if(($.browser.isIE6||$.browser.isIE9)&&self.timeline.lightboxStyle==0){self.displayForStory({story:data.story});return;}
...
self.imageViewerController.launchWithImage({story:self.selectedStory,imageIndex:self.imageGallery.selectedImageIndex,lightbox:self.timeline.lightboxStyle});}});$(domThumbRoot).mousedown(function(e){return false;});self.imageGallery=new AJKImageGallery({domRootEl:domGalleryRoot,domStageHolder:$(self.domRoot).find(".tl-g-main-content").get()[0],domStage:$(self.domRoot).find(".tl-g-main-stage").get()[0],transition:transition,domPrevButton:$(self.domRoot).find(".tl-g-gallery-control-left").get()[0],domNextButton:$(self.domRoot).find(".tl-g-gallery-control-right").get()[0],buttonHideClass:"tl-hide",imageInfoFunction:"",imageHolderHTML:'<div class="tl-g-main-item ak-image-holder"></div>',thumbBlockSize:{height:60,width:60,widthPadding:3},createImageInfoHTML:function(data){var anImage=data.image;var insertHTML="";if(anImage.caption){insertHTML+='<div class="tl-g-caption-holder">';insertHTML+='<p>'+AJKHelpers.removeScript({content:anImage.caption})+'</p>';insertHTML+='</div>';}
...
return insertHTML;},createDomImage:function(data){var anImage=data.image;var insertHTML="";if(anImage.type=="Image"){var imageSrc=AJKHelpers.getTimelineImageUrl({src:anImage.src,emptyImage:self.emptyImageReplacer});insertHTML+='<img origSrc="'+anImage.src+'" src="'+imageSrc+'" alt="'+anImage.caption+'" />';}
...
return $(insertHTML).get()[0];},domThumbPrevButton:$(self.domRoot).find(".tl-g-thumb-control-left").get()[0],domThumbNextButton:$(self.domRoot).find(".tl-g-thumb-control-right").get()[0],selectedThumbClass:"tl-g-thumb-item-selected",domThumbsHolder:domThumbRoot,domThumbsStage:$(self.domRoot).find(".tl-g-thumb-stage").get()[0],createThumbBlockFunc:function(data){var domImage=data.domImage;var insertHTML='<div class="tl-g-thumb-item">';insertHTML+='<div class="tl-g-thumb-mask"></div>';insertHTML+='</div>';var domThumb=$(insertHTML).get()[0];$(domThumb).prepend(domImage);return domThumb;},resizeCallbackFunc:function(data){var newSize=data.newSize;$(self.domGalleryMaskInner).css({height:newSize.height-2});},imageWasSelected:function(data){var origSrc=$(data.anImage.domImage).attr("origSrc");var domGStage=self.imageGallery.domStageHolder;if(origSrc&&origSrc.indexOf("flickr.com")!=-1){$(domGStage).addClass("tl-g-make-item-clickable");}
...
theAJKWindowResizeEvent.removeAsObserver({observer:self});theAJKWindowResizeEvent.registerAsObserver({observer:self});self.resizeContent({newSize:AJKHelpers.viewportSize()});setTimeout(function(){self.resizeContent({newSize:AJKHelpers.viewportSize()});});self.contentScroller.reset();self.contentScroller.resetSize();if(!story.isTimelineIntro){theTLHashController.setHashToStoryPanel({storyId:story.id});}},removeVideo:function(){var self=this;$(self.domVideoContent).empty();setTimeout(function(){$(self.domVideoContentHolder).css({display:"none"});$(self.domContentViewport).removeClass(self.showVideoClass);},1);},hide:function(){var self=this;theAJKWindowResizeEvent.removeAsObserver({observer:self});if($.browser.msie&&$.browser.version<9){$(self.domRoot).css({display:"none"});self.removeVideo();}
...
$(self.domTextContentBlock).css({width:self.contentViewportWidth-self.currentGalleryWidth-1});if(self.imageGallery&&showGallery){var setUpGallery=function(){var galleryHeight=contentHeight-51-self.thumbBlockHeight;var forceHideThumbs=false;if(self.selectedStory.isTimelineIntro||self.imageGallery.numImages<=1){forceHideThumbs=true;}
...
self.contentScroller.resetSize();},formatBodyText:function(data){var self=this;var someText=data.text;var extraLinkText=(self.timeline.embed=="true")?'target="_top"':"";extraLinkText=(self.timeline.openReadMoreLinks==1)?'target="_blank"':extraLinkText;if(someText){someText=someText.replace(/\\'/g,"'").replace(/;xNLx;/g,"\n").replace(/\r\n/g,"\n").replace(/\r/g,"\n").replace(/\n\n/g,"\n").replace(/   /g," ").replace(/  /g," ").replace(/ \n/g,"\n").replace(/\n/g,"</p><p>");someText='<p>'+someText+'</p>';var aMatch="";while((aMatch=someText.match(/\[([^-\[\(\]\)]*)\]\(([^\[\(\]\)]*)\)/))){var aLink=AJKHelpers.customCreateClickableLinks({aString:aMatch[0],extra:extraLinkText});someText=someText.replace(aMatch[0],aLink);}}
return someText;}}
...
TLCPImageViewer.prototype={init:function(){var self=this;$(self.domClose).click(function(){self.hide();return false;});return self;},launchWithImage:function(data){var self=this;if(self.active){return;}
...
$(self.controller.domContentViewport).animate({opacity:0},500,function(){$(this).css({display:"none"});$(self.domImage).animate({height:endSize.height,width:endSize.width,top:0,left:0},750);$(self.domRoot).animate({width:endSize.width,height:endSize.height,left:newLeft,top:newTop},750,function(){self.animating=false;theAJKWindowResizeEvent.registerAsObserver({observer:self});});$(self.domMask).animate({height:endSize.height-2},750);});},10);}},adjustImageSize:function(data){var self=this;var minSize=320;var imageSize=data.imageSize;var retSize={};if(imageSize.width>=imageSize.height&&imageSize.height<minSize){retSize.height=minSize;retSize.width=Math.round(imageSize.width*minSize/imageSize.height);}
...
$(self.domRootEl).find(".ajk-pd-back-to-start").click(function(){self.jumpTo({pos:0});return false;});$(self.domForgottenPassword).click(function(){self.jumpTo({pos:1});return false;});self.loginVerification=new AJKVerifier({domRootEl:$(self.domRootEl).find(".fp-block-0").get()[0],submitFunc:function(data){data.fieldData["password"]=data.fieldData["tikitokipassword"];theAJKAjaxController.request({action:"/login/",method:"post",alwaysAllow:true,vars:data.fieldData,callback:function(xml){var error=$(xml).find("error").text();if(error){self.loginVerification.errorHighlightField({fieldName:"username"});self.loginVerification.errorHighlightField({fieldName:"tikitokipassword"});self.loginVerification.displayErrorMessage({message:TLConfigText['loginPopdown_incorrectLogin_message']});}
...
self.setValue=function(data){$(this.domControl).val(AJKHelpers.removeScript({content:data.value}));}
break;}},calculateControlType:function(){var self=this;var tagName=self.domControl.tagName;var type=$(self.domControl).attr("type");self.controlType=(tagName+" "+type).toLowerCase();if(tagName=="SELECT"){self.controlType="select";}
...
return limit;},mouseMoveFunc:function(data){var dragElPos=data.dragElPos;self.updateCarouselFromScrollBarPos({scrollBarPos:dragElPos});}}).init();}}
...
var theAJKWindowResizeEvent,theTLSettings,theTLMainController,theAJKMouseMoveEvent,theAJKMouseScrollEvent,theAJKButtonPopupController,theAJKKeyEvent,theAJKAjaxController,theTLHashController;$(document).ready(function(){$.browser.isChrome=navigator.userAgent.toLowerCase().indexOf('chrome')>-1;$.browser.isMac=navigator.userAgent.toLowerCase().indexOf('macintosh')>-1;$.browser.isWindows=navigator.userAgent.toLowerCase().indexOf('windows')>-1;$.browser.isIOS=(navigator.userAgent.toLowerCase().match(/(iphone|ipod|ipad)/))?true:false;if($.browser.msie&&$.browser.version>=9){$.browser.msie=false;$.browser.isIE9=true;}
if($.browser.msie&&$.browser.version==6){$.browser.isIE6=true;}
if($.browser.isIOS){$("body").addClass("tl-browser-type-ios");}
theAJKWindowResizeEvent=new AJKWindowResizeEvent().init();theAJKMouseMoveEvent=new AJKMouseMoveEvent().init();theAJKMouseScrollEvent=new AJKMouseScrollEvent({disableScrollPropagation:true}).init();theAJKKeyEvent=new AJKKeyEvent().init();theAJKWindowBlurEvent=new AJKWindowBlurEvent().init();theAJKAjaxController=new AJKAjaxController().init();theAJKButtonPopupController=new AJKButtonPopupController({domButtons:$("#tl-header .main-menu a").get()}).init();theTLSettings=new TLSettings().init();theTLHashController=new TLHashController().init();(function(){if($("#tl-slider-scale canvas").get()[0].getContext){theTLMainController=new TLMainController();theTLMainController.init();}
...
if(isTouchDevice){var touchHandler=function(event){var touches=event.changedTouches,first=touches[0],type="";switch(event.type){case"touchstart":type="mousedown";break;case"touchmove":type="mousemove";break;case"touchend":type="mouseup";break;default:return;}
